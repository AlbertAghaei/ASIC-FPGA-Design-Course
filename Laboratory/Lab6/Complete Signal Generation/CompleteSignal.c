...
SignalType current_signal = SIGNAL_SINE;
...
void GpioHandler(void *CallbackRef)
{
    ...
    if (IntrStatus & GlobalIntrMask2) {
    	u32 key_val = XGpio_DiscreteRead(GpioPtr, GPIO_CHANNEL2);
    	//xil_printf("%d \n",key_val);
    	        switch (key_val) {
    	            case 0b110:  if (freq_multiplier < 16)freq_multiplier++; break;        // Increase freq
    	            case 0b101:  if (freq_multiplier > 1) freq_multiplier--; break; // Decrease freq
    	            case 0b011:  current_signal = (current_signal + 1) % SIGNAL_COUNT; break; // Next waveform
    	        }
...
//    if(flag == 1){
    ...
    //xil_printf("Generating wave: %d, Freq x%d\n", current_signal, freq_multiplier);
  /*  for (int i = 0; i < (512 ) ; i++)
   {
...
int generate_waveform(u32 Start_Addr, u32 Dest_Addr, u32 Len, SignalType type) {
    ...
    for (int i = 0; i < Len; i++) {
        ...
        switch (type) {
            case SIGNAL_SINE:
                val = sin(2 * PI * t); break;
            case SIGNAL_SAWTOOTH:
                val = 2.0 * t - 1.0; break;
            case SIGNAL_SQUARE:
                val = t < 0.5 ? 1.0 : -1.0; break;
            case SIGNAL_TRIANGLE:
                val = 4.0 * fabs(t - 0.5) - 1.0; break;
            default:
                val = 0.0;
        }

        uint16_t fixed_val = (uint16_t)(int16_t)(val * 16384);
...
int process_wave(u32 Start_Addr, u32 Dest_Addr, u32 Len, int freq_mult) {
   ...
   // Configure PL-PS BRAM interconnect registers
    PL_PS_BRAM_INTERCONNECT_mWriteReg(PL_RAM_BASE, PL_RAM_REG1, freq_mult);
    PL_PS_BRAM_INTERCONNECT_mWriteReg(PL_RAM_BASE, PL_RAM_REG2, Len);
    PL_PS_BRAM_INTERCONNECT_mWriteReg(PL_RAM_BASE, PL_RAM_REG3, BRAM_BYTENUM*Start_Addr);
    PL_PS_BRAM_INTERCONNECT_mWriteReg(PL_RAM_BASE, PL_RAM_REG4, BRAM_BYTENUM*Dest_Addr);
...
int main()
  ...
  status = GpioSetupIntrSystem(&INTCInst , &PLGpio , GPIO_DEVICE_ID , GPIO_INTR_ID , 1 , 2 );   // (INTC *IntcInstancePtr, XGpio *InstancePtr, u16 DeviceId, u16 IntrId, u16 IntrMask1, u16 IntrMask2)
  if (status != XST_SUCCESS)
       return XST_FAILURE;
  ...
  readBuffer = (float*) malloc(Len * sizeof(float));
  ...
  while (1) {
	  if (Gpio2_flag) {
		  Gpio2_flag = 0;
		  status = generate_waveform(Start_Addr,Dest_Addr, Len, current_signal);
		  if(status != XST_SUCCESS) {
			  xil_printf("failed!\n\r");
		  }

		  status = process_wave(Start_Addr, Dest_Addr, Len, freq_multiplier);
		  if(status != XST_SUCCESS) {
			  xil_printf("failed!\n\r");
		  }
	  }
	  ...
	  if(Gpio1_flag) {
		  ...
		  int counter = 0;
		  for (int i = BRAM_BYTENUM * Dest_Addr; i <  (BRAM_BYTENUM * (Dest_Addr + Len)); i+= BRAM_BYTENUM) {
				  readData = XBram_ReadReg(XPAR_BRAM_0_BASEADDR, i);
				  int16_t val = (int16_t)(readData & 0x0000FFFF);
				  float real_val = ((float)val / (16384.0));
				  readBuffer[counter++] = real_val;
		  }
	  }
	  else {
		  for (int i = 0; i < Len; i++) {
